...
class UserTest(unittest.TestCase):
    ...
    def create_blog(self):
        return self.app.post('/setup/', data=dict(
            name="My Test Blog",
            fullname="Fadzil Jusri",
            email="fadziljusri@gmail.com",
            username="fadzil",
            password="thisisnottherealpassword",
            confirm="thisisnottherealpassword"
            ),
    ...
    def test_login_logut(self): # will give error since the password is not correct
        self.create_blog()
        rv = self.login('fadzil', 'thisisnottherealpassword')
        assert 'User fadzil logged in' in str(rv.data)
        rv = self.logout()
